...
TEST(SchnorrGuards, RejectsBitFlipAndWrongMessage)
{
    const std::array<uint8_t,32> pubkey = {
        0xF9,0x30,0x8A,0x01,0x92,0x58,0xC3,0x10,0x49,0x34,0x4F,0x85,0xF8,0x9D,0x52,0x29,
        0xB5,0x31,0xC8,0x45,0x83,0x6F,0x99,0xB0,0x86,0x01,0xF1,0x13,0xBC,0xE0,0x36,0xF9};
    ...
    const auto compressed = ToCompressedPub(pubkey);
    ASSERT_TRUE(schnorr_verify(compressed.data(), msg.data(), sig.data()));

    auto flipped = sig;
    flipped[5] ^= 0x01;
    EXPECT_FALSE(schnorr_verify(compressed.data(), msg.data(), flipped.data()));

    auto wrongMsg = msg;
    wrongMsg[0] ^= 0x80;
    EXPECT_FALSE(schnorr_verify(compressed.data(), wrongMsg.data(), sig.data()));

    const bool first = schnorr_verify(compressed.data(), msg.data(), sig.data());
    const bool second = schnorr_verify(compressed.data(), msg.data(), sig.data());
    EXPECT_TRUE(first);
    EXPECT_EQ(first, second);
}

TEST(SchnorrGuards, RejectsMalformedPublicKeysAndScalarEdges)
{
    std::array<uint8_t,32> msg{};
    std::array<uint8_t,64> sig{};
    sig.fill(0xAA);
    ...
    std::array<uint8_t,33> nonCanonical{};
    nonCanonical.fill(0x05); // invalid SEC1 prefix
    EXPECT_FALSE(schnorr_verify(nonCanonical.data(), msg.data(), sig.data()));

    std::array<uint8_t,33> offCurve{};
    offCurve.fill(0xFF);
    offCurve[0] = 0x02; // x not in field
    EXPECT_FALSE(schnorr_verify(offCurve.data(), msg.data(), sig.data()));
    ...
    const std::array<uint8_t,64> good_sig = {
        ...
        0x89,0x06,0xD1,0x1A,0xC9,0x76,0xAB,0xCC,0xB2,0x0B,0x09,0x12,0x92,0xBF,0xF4,0xEA,
        0x89,0x7E,0xFC,0xB6,0x39,0xEA,0x87,0x1C,0xFA,0x95,0xF6,0xDE,0x33,0x9E,0x4B,0x0A};
    const std::array<uint8_t,32> msg_valid = {
        0x24,0x3F,0x6A,0x88,0x85,0xA3,0x08,0xD3,0x13,0x19,0x8A,0x2E,0x03,0x70,0x73,0x44,
        0xA4,0x09,0x38,0x22,0x29,0x9F,0x31,0xD0,0x08,0x2E,0xFA,0x98,0xEC,0x4E,0x6C,0x89};
    const auto compressed = ToCompressedPub(pubkey);
    ASSERT_TRUE(schnorr_verify(compressed.data(), msg_valid.data(), good_sig.data()));

    auto zeroS = good_sig;
    std::fill(zeroS.begin() + 32, zeroS.end(), 0x00);
    EXPECT_FALSE(schnorr_verify(compressed.data(), msg_valid.data(), zeroS.data()));

    auto atOrder = good_sig;
    std::copy(kCurveOrder.begin(), kCurveOrder.end(), atOrder.begin() + 32);
    EXPECT_FALSE(schnorr_verify(compressed.data(), msg_valid.data(), atOrder.data()));
}
